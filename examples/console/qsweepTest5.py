...
def qSweep(minFreq, maxFreq, stepFreq):
    ...
    for freq in frequencies:
        ...
        try:
            ul.a_out_scan(board_num, low_chan, high_chan, totalPoints, rate, ao_range, scanBuffer, ScanOptions.EXTTRIGGER.BACKGROUND)
            print('test')
            ul.a_in_scan(board_num, low_chan, high_chan, inputPoints, inRate, ai_range, inputBuffer, ScanOptions.EXTTRIGGER)
            print('test2')
        ...
        ul.a_out(board_num, trig_channel, ao_range, 0) # Reset output to -10 V
